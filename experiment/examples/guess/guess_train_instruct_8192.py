...
def load_model_for_training(mode: Mode, max_turn_length: int, max_conversation_length: int):
    ...
    if mode == ModeDebug:
        ...
        deepspeed = None
        model, processing_class = load_model_and_tokenizer(model_path, attn_implementaion="sdpa")
    else:
        model, processing_class = load_model_and_tokenizer(model_path, attn_implementaion="flash_attention_2")

    # prevent TRL from using apply_chat_template
    processing_class.apply_chat_template = fail_apply_chat_template
...
def load_env_and_data(effective_batch_size: int):
    from experiment.examples.guess.guess_env import SYSTEM_PROMPT, generate_seed
    from experiment.examples.guess.guess_env import GuessEnv
