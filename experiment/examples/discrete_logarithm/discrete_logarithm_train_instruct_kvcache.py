...
from trl_env.rollout_transformer_kvcache import TransformerEngine, make_reward_func, make_rollout_func
...
def load_model(mode: Mode, max_turn_length: int, max_conversation_length: int):
    ...
    engine = TransformerEngine(
        ...
        extra_eos_tokens=[tokenizer(token).input_ids[0] for token in EXTRA_EOS_TOKEN_LIST]
    )
...
def main(mode: Mode, uuid: str):
    ...
    output_dir = f"mnt/output/discrete-logarithm-{os.path.basename(model_path)}-instruct-tl{max_turn_length}-cl{max_conversation_length}-b{effective_batch_size}-{uuid}-lora"
