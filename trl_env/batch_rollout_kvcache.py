...
from .model_kvcache import ModelWithKVCache
...
def batch_rollout_with_kvcache(
    model: ModelWithKVCache, processor: Processor, env_factory: Callable[[], Env],
    ...
    last_processed_indices = [0] * len(state_list)
    ...
    with ThreadPoolExecutor(max_workers=len(env_list)) as executor:
        ...
        while sum([env.alive for env in env_list]) > 0:
            # Determine new tokens to send to model
            # For dead environments, we pass a pad token to keep batch consistent
            new_input_ids_list = [
                state.conversation[last_processed_indices[i]:] if env.alive else [model.pad_token_id]
                for i, (env, state) in enumerate(zip(env_list, state_list))
            ]

            # MODEL BATCH GENERATE
            # i.e. give the model the last hidden states so that it won't recalculate everything from the beginning
            completion_ids_list, logprobs_list, past_key_values = model.model_batch_generate(
                new_input_ids_list,
                past_key_values=past_key_values
            )

            # PROCESS GENERATE
            def process_generate(i: int, env: Env, state: RolloutState, completion_ids: list[int], logprobs: list[float]) -> tuple[int, Env, RolloutState, int]:
                # precheck env.alive
                if not env.alive:
                    return i, env, state, last_processed_indices[i]
                ...
                # AFTER completion is added, the cache is up to date with state.conversation
                # so the next chunk we need to send is what comes after this point (the delta)
                next_last_processed_index = len(state.conversation)

                # postcheck env.alive
                if not env.alive:
                    LOG(i, "log", "env terminated")
                    return i, env, state, next_last_processed_index
            ...
            for i, env, state, next_idx in output_iter:
                env_list[i] = env
                state_list[i] = state
                last_processed_indices[i] = next_idx
...
from .model_kvcache import TransformerModelWithKVCache
import torch
...
def make_rollout_func(
    ...
    def rollout_func(prompts: list[str], trainer: GRPOTrainer) -> dict[str, Any]:
        try:
            ...
            model.model.eval()
            with torch.no_grad():
                state_list = batch_rollout_with_kvcache(
